...
def token(user: User) -> str:
    payload = {"sub": user.id, "org": user.org_id, "role": user.role, "exp": now() + timedelta(hours=JWT_HOURS), "jti": uuid.uuid4().hex}
...
def auth_api_key(x_api_key: str | None = Header(None)) -> dict[str, Any]:
    ...
    with db() as s:
        ...
        k.last_used = now(); s.commit()
        return {"org": k.org_id, "api_key_id": k.id, "role": "api_key", "scopes": k.scopes}

def auth(authorization: str | None = Header(None)) -> dict[str, Any]:
    if not authorization or not authorization.startswith("Bearer "): raise HTTPException(401, "Authentication required")
    try: return jwt.decode(authorization[7:], JWT_SECRET, algorithms=[JWT_ALG])
...
class AgentIn(BaseModel): name: str = Field(min_length=1, max_length=200); model: str = "Customer model"; owner: str = "Owner"
class PolicyIn(BaseModel): name: str; action: str = "*"; resource: str = "*"; effect: str = "BLOCK"; threshold: float = 0
class GatewayIn(BaseModel): agent_id: str; action: str; resource: str = "*"; amount: float = 0; details: dict[str, Any] = {}
class ApprovalDecision(BaseModel): status: str = Field(pattern="^(approved|rejected)$")
class MfaCode(BaseModel): code: str = Field(min_length=6, max_length=8)
class OAuthCode(BaseModel): code: str
class CheckoutIn(BaseModel): price_id: str
...
def _fernet():
    ...
    if not key: raise HTTPException(503, "INTEGRATION_ENCRYPTION_KEY is not configured")
    return Fernet(key.encode())

def _oauth_config(provider: str):
    configs={
      "google":{"client_id":os.getenv("GOOGLE_CLIENT_ID"),"client_secret":os.getenv("GOOGLE_CLIENT_SECRET"),"auth":"https://accounts.google.com/o/oauth2/v2/auth","token":"https://oauth2.googleapis.com/token","userinfo":"https://openidconnect.googleapis.com/v1/userinfo","scope":"openid email profile"},
      "slack":{"client_id":os.getenv("SLACK_CLIENT_ID"),"client_secret":os.getenv("SLACK_CLIENT_SECRET"),"auth":"https://slack.com/oauth/v2/authorize","token":"https://slack.com/api/oauth.v2.access","userinfo":"https://slack.com/api/users.identity","scope":"openid,email,profile"}
    }
    c=configs.get(provider)
    if not c or not c["client_id"] or not c["client_secret"]: raise HTTPException(503, f"{provider} OAuth is not configured")
    return c

def send_email(to:str, subject:str, body:str):
    host=os.getenv("SMTP_HOST"); port=int(os.getenv("SMTP_PORT","587")); user=os.getenv("SMTP_USERNAME"); password=os.getenv("SMTP_PASSWORD"); sender=os.getenv("SMTP_FROM",user or "")
...
@app.get("/health")
def health(): return {"status":"ok", "service":"AgentGuard", "version":"2.0.0", "time":now().isoformat()}

@app.get("/api/oauth/{provider}/start")
def oauth_start(provider:str):
    c=_oauth_config(provider); redirect=os.getenv("OAUTH_REDIRECT_BASE","http://localhost:8000")+f"/api/oauth/{provider}/callback"
...
def oauth_callback(provider:str, code:str, state:str=""):
    ...
    try:
        ...
        if state_claims.get("provider") != provider: raise ValueError()
    except Exception:
    ...
    with db() as s:
        ...
        if not u:
            u=User(id=uid(),email=email.lower(),password_hash=pwd.hash(secrets.token_urlsafe(24)),role="owner",org_id=uid()); s.add(u); s.commit(); s.refresh(u)
...
def mfa_login(x: MfaCode, mfa_token: str = Header(..., alias="X-MFA-Token")):
    pyotp=_mfa_lib()
    try: claims=jwt.decode(mfa_token,JWT_SECRET,algorithms=[JWT_ALG])
    except jwt.PyJWTError: raise HTTPException(401,"Invalid or expired MFA challenge")
    if claims.get("purpose")!="mfa": raise HTTPException(401,"Invalid MFA challenge")
    with db() as s:
        u=s.scalar(select(User).where(User.id==claims["sub"],User.org_id==claims["org"]))
        if not u or not u.mfa_secret or not pyotp.TOTP(u.mfa_secret).verify(x.code,valid_window=1): raise HTTPException(401,"Invalid MFA code")
        return {"access_token":token(u),"mfa_required":False,"user":{"id":u.id,"email":u.email,"role":u.role,"org_id":u.org_id}}
...
def mfa_setup(u=Depends(auth)):
    ...
    with db() as s:
        ...
        secret=pyotp.random_base32(); user.mfa_secret=secret; s.commit()
...
def add_agent(x: AgentIn, u=Depends(require_role("owner","admin","developer"))):
    with db() as s:
        a=Agent(id=uid(),name=x.name,model=x.model,owner=x.owner,org_id=u["org"]); s.add(a); s.commit(); audit(s,u,a.id,"agent.create","ALLOW",x.name); return {"id":a.id}
...
def kill(aid: str, u=Depends(require_role("owner","admin","security"))):
    with db() as s:
        a=s.scalar(select(Agent).where(Agent.id==aid,Agent.org_id==u["org"]))
        if not a: raise HTTPException(404,"Agent not found")
        a.status="killed"; s.commit(); audit(s,u,aid,"agent.kill","BLOCK","Emergency kill switch"); return {"status":"killed"}

@app.get("/api/policies")
def policies(u=Depends(auth)):
    with db() as s: return [x.__dict__ | {} for x in s.scalars(select(Policy).where(Policy.org_id==u["org"]).order_by(Policy.created_at.desc())).all()]
...
def create_key(name: str="Agent key", scopes: str="gateway:check", u=Depends(require_role("owner","admin","developer"))):
    ...
    with db() as s:
        k=ApiKey(id=uid(),name=name,key_hash=hashlib.sha256(raw.encode()).hexdigest(),scopes=scopes,org_id=u["org"]); s.add(k); s.commit(); audit(s,u,"","api_key.create","ALLOW",name); return {"id":k.id,"key":raw,"warning":"Store this key now. It will not be shown again."}
...
def export(u=Depends(auth)):
    with db() as s:
        ...
        for model,name in [(Agent,"agents"),(Policy,"policies"),(Audit,"audit"),(Approval,"approvals"),(Incident,"incidents"),(ApiKey,"api_keys"),(Integration,"integrations")]:
            rows=s.scalars(select(model).where(model.org_id==u["org"])).all(); out[name]=[{k:v for k,v in r.__dict__.items() if k!="_sa_instance_state"} for r in rows]
        return out

@app.delete("/api/account")
def delete_account(u=Depends(auth)):
    with db() as s:
        for model in [Agent,Policy,Audit,Approval,Incident,ApiKey,Integration,User]: s.execute(delete(model).where(model.org_id==u["org"]))
        s.commit(); return {"deleted":True}

def scim_auth(token_value: str | None):
    expected=os.getenv("SCIM_BEARER_TOKEN")
    if not expected or token_value != expected: raise HTTPException(401,"Invalid SCIM bearer token")
...
def scim_users(authorization: str | None = Header(None)):
    ...
    if not org: raise HTTPException(503,'SCIM_ORG_ID is not configured')
    with db() as s:
...
def scim_create(payload: dict, authorization: str | None = Header(None)):
    scim_auth(authorization.replace('Bearer ','',1) if authorization else None)
    org=os.getenv('SCIM_ORG_ID'); email=str(payload.get('userName','')).lower()
    if not org or not email: raise HTTPException(400,'SCIM_ORG_ID and userName are required')
    with db() as s:
        if s.scalar(select(User).where(User.email==email)): raise HTTPException(409,'User already exists')
        u=User(id=uid(),email=email,password_hash=pwd.hash(secrets.token_urlsafe(32)),role='viewer',org_id=org); s.add(u); s.commit(); s.refresh(u)
...
def billing_checkout(x: CheckoutIn, u=Depends(require_role('owner','admin'))):
    ...
    if not key: raise HTTPException(503,'STRIPE_SECRET_KEY is not configured')
    stripe.api_key=key
    try:
        session=stripe.checkout.Session.create(mode='subscription',line_items=[{'price':x.price_id,'quantity':1}],success_url=os.getenv('STRIPE_SUCCESS_URL','http://localhost:8000/?billing=success'),cancel_url=os.getenv('STRIPE_CANCEL_URL','http://localhost:8000/?billing=cancel'),metadata={'org_id':u['org']})
...
async def billing_webhook(request: Request, stripe_signature: str | None = Header(None, alias='Stripe-Signature')):
    ...
    if not secret: raise HTTPException(503,'STRIPE_WEBHOOK_SECRET is not configured')
